package com.jandy.akkapractice.package9.http.dsl
      ...
      content.split(",") match {
        case Array(location, _)
          if cache.contains(location) =>
            StatusCodes.Conflict -> s"$location has a value already, use a PUT method if you wish to update it"
...
trait PutRequestsHandler {
  def handlePut(cache: scala.collection.mutable.Map[String,
