package com.jandy.akkapractice.package9.http.json
...
object OrderCalculatorJsonServer extends HttpApp with OrderJsonSupport {
  ...
  private def calculateGrandTotal(o: Order) ={
    val amount = o.items.map(
      i=> i.percentageDiscount.getOrElse(1.0d) * i.unitPrice * i.quantity).sum + o.deliveryPrice
...
object OrderCalculatorJsonApplication extends App{
